...
def eval_func(individual):
    ...
    for x in range(1, 11):  # Evaluate for x from 1 to 10
        error += abs(5*x**3 - 6*x**2 + 8*x - 1 - func(x))
    return error / 10,  # Return the average error


# Create the toolbox with the right parameters
def create_toolbox(num_primitives):
    ...
    pset.addPrimitive(add, arity=2)
    pset.addPrimitive(sub, arity=2)
    pset.addPrimitive(mul, arity=2)
...
if __name__ == "__main__":
    ...
    toolbox = create_toolbox(num_primitives)
    random.seed(7)
